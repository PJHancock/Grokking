...
class Transformer(nn.Module):
    ...
    def forward_hidden(self, x, attn_mask, past_kvs=None):
        # x = (batch, time)
        ...
        if self.pre_norm:
            x = self.norm(x)
        return x, attns, new_past_kvs

    def forward(self, x, attn_mask, past_kvs=None):
        hidden_states, attns, new_past_kvs = self.forward_hidden(x, attn_mask, past_kvs=past_kvs)
        return self.output(hidden_states), attns, new_past_kvs
